# Generated Link-Building Outreach Packets

Generated: 2026-06-25

These packets are generated from `docs/link-building/prospect-tracker.csv`. They are not sent mail. Review each prospect manually before outreach.

| Priority | Prospect | Segment | Target | Asset | Next action |
| --- | --- | --- | --- | --- | --- |
| 1 | GitHub profile | owned-profile | https://sulayman-bowles.dev/atlas | Atlas project | Update profile manually |
| 1 | Portfolio_Site repository | owned-profile | https://sulayman-bowles.dev/ | Portfolio static SEO implementation | Update repo metadata manually |
| 1 | Thick-Scraper-VOID- repository | owned-profile | https://sulayman-bowles.dev/atlas/sample-crawl | Atlas sample crawl | Update repo metadata manually |
| 1 | LinkedIn profile | owned-profile | https://sulayman-bowles.dev/resume | Resume and source graph | Update profile manually |
| 1 | UT Austin Student Government | existing-mention | https://sulayman-bowles.dev/ai-information | AI information source graph | Confirm mention and contact owner |
| 1 | McCombs School of Business | academic | https://sulayman-bowles.dev/resume | Resume | Find relevant program/profile owner |
| 1 | Texas Venture Labs | academic | https://sulayman-bowles.dev/atlas | Atlas project | Verify eligibility and contact path |
| 1 | Search Engine Land | technical-seo | https://sulayman-bowles.dev/markets/ai-search-crawler-policy | AI crawler policy | Find contributor/editor contact |
| 1 | Search Engine Journal | technical-seo | https://sulayman-bowles.dev/markets/ai-search-crawler-policy | AI crawler policy | Find contributor/editor contact |
| 1 | SEOFOMO | technical-seo | https://sulayman-bowles.dev/markets/ai-search-crawler-policy | AI crawler policy | Find submission/contact path |
| 1 | Detailed | technical-seo | https://sulayman-bowles.dev/markets/technical-seo-public-data-infrastructure | Public data infrastructure | Research fit and contact |
| 1 | TechnicalSEO.com | technical-seo | https://sulayman-bowles.dev/atlas/sample-crawl | Atlas sample crawl | Research submission path |
| 1 | OpenAI developer community | ai-search | https://sulayman-bowles.dev/markets/ai-search-crawler-policy | AI crawler policy | Post only if community rules allow |
| 1 | Perplexity docs/community | ai-search | https://sulayman-bowles.dev/markets/ai-search-crawler-policy | AI crawler policy | Research discussion channels |
| 1 | Anthropic support/community | ai-search | https://sulayman-bowles.dev/markets/ai-search-crawler-policy | AI crawler policy | Research discussion channels |
| 1 | Austin Chamber | local-austin | https://sulayman-bowles.dev/austin-technical-seo | Austin technical SEO | Check member/directory requirements |
| 1 | Built In Austin | local-austin | https://sulayman-bowles.dev/austin-technical-seo | Austin technical SEO | Research contributor/company profile options |
| 1 | Austin Business Journal | local-austin | https://sulayman-bowles.dev/austin-technical-seo | Austin benchmark | Prepare benchmark before pitch |

## Packet 1: GitHub profile

- Status: owned
- Priority: 1
- Segment: owned-profile
- Prospect URL: https://github.com/SulaymanB2024
- Target page: https://sulayman-bowles.dev/atlas
- Asset: Atlas project
- Angle: Add canonical site and Atlas link to profile bio/sidebar
- Next action: Update profile manually
- Template: Atlas Evidence Pack
- Live evidence: Not yet verified in `docs/link-building/live-prospect-evidence.csv`.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 2: Portfolio_Site repository

- Status: owned
- Priority: 1
- Segment: owned-profile
- Prospect URL: https://github.com/SulaymanB2024/Portfolio_Site
- Target page: https://sulayman-bowles.dev/
- Asset: Portfolio static SEO implementation
- Angle: Add homepage link and concise technical SEO/source-graph description
- Next action: Update repo metadata manually
- Template: AI Crawler Policy Resource
- Live evidence: Not yet verified in `docs/link-building/live-prospect-evidence.csv`.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 3: Thick-Scraper-VOID- repository

- Status: owned
- Priority: 1
- Segment: owned-profile
- Prospect URL: https://github.com/SulaymanB2024/Thick-Scraper-VOID-
- Target page: https://sulayman-bowles.dev/atlas/sample-crawl
- Asset: Atlas sample crawl
- Angle: Point README and repo metadata at sample crawl proof asset
- Next action: Update repo metadata manually
- Template: Atlas Evidence Pack
- Live evidence: Not yet verified in `docs/link-building/live-prospect-evidence.csv`.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 4: LinkedIn profile

- Status: owned
- Priority: 1
- Segment: owned-profile
- Prospect URL: https://www.linkedin.com/in/sulayman-bowles/
- Target page: https://sulayman-bowles.dev/resume
- Asset: Resume and source graph
- Angle: Use the current identity line and link to resume/source graph
- Next action: Update profile manually
- Template: AI Crawler Policy Resource
- Live evidence: Not yet verified in `docs/link-building/live-prospect-evidence.csv`.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 5: UT Austin Student Government

- Status: research
- Priority: 1
- Segment: existing-mention
- Prospect URL: https://studentgovernment.utexas.edu/legislative-branch
- Target page: https://sulayman-bowles.dev/ai-information
- Asset: AI information source graph
- Angle: Current source link for identity reconciliation
- Next action: Confirm mention and contact owner
- Template: Profile / Existing Mention Reclamation
- Live evidence: Legislative Branch page currently lists University-Wide: Sulayman Bowles with a Contact link.
- Source checked: https://studentgovernment.utexas.edu/legislative-branch
- Contact/submission path: https://studentgovernment.utexas.edu/legislative-branch
- Fit status: existing_mention
- Evidence-based next action: No link request unless profile/update path legitimately exists; use as corroborating public source.
- Evidence boundary: Do not expose personal contact details in outreach notes.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 6: McCombs School of Business

- Status: research
- Priority: 1
- Segment: academic
- Prospect URL: https://www.mccombs.utexas.edu/
- Target page: https://sulayman-bowles.dev/resume
- Asset: Resume
- Angle: Legitimate profile or venture/student-source link if available
- Next action: Find relevant program/profile owner
- Template: AI Crawler Policy Resource
- Live evidence: Brumley Institute page lists McCombs entrepreneurship programs, resources, and contact paths.
- Source checked: https://www.mccombs.utexas.edu/centers-initiatives/brumley-institute/
- Contact/submission path: https://www.mccombs.utexas.edu/centers-initiatives/brumley-institute/
- Fit status: opportunity_if_eligible
- Evidence-based next action: Research whether a current student founder/profile path exists before asking for any link.
- Evidence boundary: Do not request generic McCombs homepage links.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 7: Texas Venture Labs

- Status: research
- Priority: 1
- Segment: academic
- Prospect URL: https://www.mccombs.utexas.edu/centers-and-initiatives/texas-venture-labs/
- Target page: https://sulayman-bowles.dev/atlas
- Asset: Atlas project
- Angle: Founder/project profile link if a real participation page exists
- Next action: Verify eligibility and contact path
- Template: Atlas Evidence Pack
- Live evidence: TVLIC page describes a University of Texas graduate-student venture competition and current application path.
- Source checked: https://www.mccombs.utexas.edu/centers-initiatives/brumley-institute/tvl-investment-competition/
- Contact/submission path: https://www.mccombs.utexas.edu/centers-initiatives/brumley-institute/tvl-investment-competition/
- Fit status: opportunity_if_eligible
- Evidence-based next action: Only pursue profile/competition link if Atlas or a related venture is eligible and submitted.
- Evidence boundary: Do not imply participation before it is public and true.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 8: Search Engine Land

- Status: research
- Priority: 1
- Segment: technical-seo
- Prospect URL: https://searchengineland.com/
- Target page: https://sulayman-bowles.dev/markets/ai-search-crawler-policy
- Asset: AI crawler policy
- Angle: Source-backed AI crawler policy pitch
- Next action: Find contributor/editor contact
- Template: AI Crawler Policy Resource
- Live evidence: Contact page accepts feature/source pitches and asks for evidence-backed claims and clear marketer relevance.
- Source checked: https://searchengineland.com/contact
- Contact/submission path: https://searchengineland.com/contact
- Fit status: ready
- Evidence-based next action: Pitch AI crawler policy as source-backed feature/source note.
- Evidence boundary: Do not pitch as guaranteed AI-search ranking or DR impact.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 9: Search Engine Journal

- Status: research
- Priority: 1
- Segment: technical-seo
- Prospect URL: https://www.searchenginejournal.com/
- Target page: https://sulayman-bowles.dev/markets/ai-search-crawler-policy
- Asset: AI crawler policy
- Angle: AI search crawler policy and technical SEO source clarity
- Next action: Find contributor/editor contact
- Template: AI Crawler Policy Resource
- Live evidence: Contributor guidelines say contributors are invited by editorial team only and ad hoc slots require pre-approved topics.
- Source checked: https://www.searchenginejournal.com/writers-guidelines-for-search-engine-journal/
- Contact/submission path: https://www.searchenginejournal.com/contact/
- Fit status: gated
- Evidence-based next action: Use contact page only for a concise source/expert pitch; do not assume guest post access.
- Evidence boundary: Do not frame as open guest-post placement.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 10: SEOFOMO

- Status: research
- Priority: 1
- Segment: technical-seo
- Prospect URL: https://seofomo.co/
- Target page: https://sulayman-bowles.dev/markets/ai-search-crawler-policy
- Asset: AI crawler policy
- Angle: Newsletter mention for AI crawler policy source map
- Next action: Find submission/contact path
- Template: AI Crawler Policy Resource
- Live evidence: SEOFOMO Hub says SEO or AI-search resources can be submitted through SEOFOMO News; submit page requires login.
- Source checked: https://hub.seofomo.co/
- Contact/submission path: https://news.seofomo.co/submit-story/
- Fit status: ready_with_login
- Evidence-based next action: Submit AI crawler policy source map after logging in and confirming community rules.
- Evidence boundary: Submission is moderated and may be nofollow or declined.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 11: Detailed

- Status: research
- Priority: 1
- Segment: technical-seo
- Prospect URL: https://detailed.com/
- Target page: https://sulayman-bowles.dev/markets/technical-seo-public-data-infrastructure
- Asset: Public data infrastructure
- Angle: Editorial pitch on technical SEO as data infrastructure
- Next action: Research fit and contact
- Template: AI Crawler Policy Resource
- Live evidence: Detailed is an SEO insights blog, but no public contact or contribution path was confirmed in this pass.
- Source checked: https://detailed.com/
- Contact/submission path: https://detailed.com/
- Fit status: research
- Evidence-based next action: Keep as high-bar editorial research target rather than immediate outreach.
- Evidence boundary: Do not add to send queue without verified contact route.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 12: TechnicalSEO.com

- Status: research
- Priority: 1
- Segment: technical-seo
- Prospect URL: https://technicalseo.com/
- Target page: https://sulayman-bowles.dev/atlas/sample-crawl
- Asset: Atlas sample crawl
- Angle: Resource/tool example for crawl evidence
- Next action: Research submission path
- Template: Atlas Evidence Pack
- Live evidence: Tools page includes technical SEO tools such as robots.txt testing and AI Bot Access Tester, but no public resource-submission path was found.
- Source checked: https://technicalseo.com/tools/
- Contact/submission path: https://technicalseo.com/
- Fit status: research
- Evidence-based next action: Keep as research target; look for a real maintainer/contact route before outreach.
- Evidence boundary: Do not infer that it accepts external tools.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 13: OpenAI developer community

- Status: research
- Priority: 1
- Segment: ai-search
- Prospect URL: https://community.openai.com/
- Target page: https://sulayman-bowles.dev/markets/ai-search-crawler-policy
- Asset: AI crawler policy
- Angle: Source-backed discussion of search vs training crawlers
- Next action: Post only if community rules allow
- Template: AI Crawler Policy Resource
- Live evidence: Guidelines allow technical discussion around OpenAI Platform/API/developer products but warn against repetitive or unrelated promotion.
- Source checked: https://community.openai.com/guidelines
- Contact/submission path: https://community.openai.com/
- Fit status: ready_if_non_promotional
- Evidence-based next action: Share only if the crawler-policy article is framed as technical discussion with developer relevance.
- Evidence boundary: Do not post a link-only promotional thread.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 14: Perplexity docs/community

- Status: research
- Priority: 1
- Segment: ai-search
- Prospect URL: https://docs.perplexity.ai/
- Target page: https://sulayman-bowles.dev/markets/ai-search-crawler-policy
- Asset: AI crawler policy
- Angle: Resource citing crawler distinctions
- Next action: Research discussion channels
- Template: AI Crawler Policy Resource
- Live evidence: Not yet verified in `docs/link-building/live-prospect-evidence.csv`.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 15: Anthropic support/community

- Status: research
- Priority: 1
- Segment: ai-search
- Prospect URL: https://support.claude.com/
- Target page: https://sulayman-bowles.dev/markets/ai-search-crawler-policy
- Asset: AI crawler policy
- Angle: Resource citing crawler distinctions
- Next action: Research discussion channels
- Template: AI Crawler Policy Resource
- Live evidence: Not yet verified in `docs/link-building/live-prospect-evidence.csv`.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 16: Austin Chamber

- Status: research
- Priority: 1
- Segment: local-austin
- Prospect URL: https://www.austinchamber.com/
- Target page: https://sulayman-bowles.dev/austin-technical-seo
- Asset: Austin technical SEO
- Angle: Local service/resource profile if legitimate
- Next action: Check member/directory requirements
- Template: Austin Crawlability Benchmark
- Live evidence: Member News page says Chamber members can submit news through the member portal.
- Source checked: https://www.austinchamber.com/member-news
- Contact/submission path: https://www.austinchamber.com/member-news
- Fit status: membership_gated
- Evidence-based next action: Use only if Void Agency or a related entity has membership access.
- Evidence boundary: Do not use as a public free-placement target.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 17: Built In Austin

- Status: research
- Priority: 1
- Segment: local-austin
- Prospect URL: https://www.builtinaustin.com/
- Target page: https://sulayman-bowles.dev/austin-technical-seo
- Asset: Austin technical SEO
- Angle: Local startup/tech profile or benchmark story
- Next action: Research contributor/company profile options
- Template: Austin Crawlability Benchmark
- Live evidence: Homepage links to Austin tech news, company pages, and a Become an Expert Contributor path; expert-contributor page was blocked to crawler fetch.
- Source checked: https://www.builtinaustin.com/
- Contact/submission path: https://builtin.com/expert-contributors
- Fit status: research_gated
- Evidence-based next action: Keep as contributor/profile research target and verify from browser/account before outreach.
- Evidence boundary: Do not treat as verified open submission.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

## Packet 18: Austin Business Journal

- Status: research
- Priority: 1
- Segment: local-austin
- Prospect URL: https://www.bizjournals.com/austin/
- Target page: https://sulayman-bowles.dev/austin-technical-seo
- Asset: Austin benchmark
- Angle: Pitch aggregate crawlability benchmark once published
- Next action: Prepare benchmark before pitch
- Template: Austin Crawlability Benchmark
- Live evidence: About page lists editorial staff and newsroom roles for Austin Business Journal.
- Source checked: https://www.bizjournals.com/austin/about-us
- Contact/submission path: https://www.bizjournals.com/austin/about-us
- Fit status: ready_story_tip
- Evidence-based next action: Pitch Austin crawlability benchmark only after benchmark exists with local data and no private shaming.
- Evidence boundary: Do not pitch a thin service page as news.
- Send status: not sent.
- Approval note: send manually only after confirming fit, contact path, and account permission.
- Claim boundary: do not imply rankings, traffic, revenue, AI citations, or DR movement.

