...
const supportingArtifacts = [
  {
    ...
    description: 'A sanitized example that moves from observed field to diagnosis, repair, and verification. It is not a client-outcome case study.',
  },
  {
  ...
  },
  {
    ...
    href: '/method',
    description: 'Scope, stages, deliverables, inputs, exclusions, and verification steps.',
  },
  {
...
function fieldLabel(label: string) {
  return <dt className="text-[11px] uppercase tracking-[0.22em] text-canvas/60">{label}</dt>;
}
  ...
  return (
    <main id="top" className="site-page site-page-dark relative min-h-screen overflow-x-clip bg-ink font-sans text-canvas selection:bg-canvas selection:text-ink">
      ...
      <section className="relative z-10 mx-auto grid min-h-[62vh] max-w-[1480px] content-end px-4 pb-16 pt-20 md:px-8 xl:px-10">
        <p className="mb-8 text-[11px] uppercase tracking-[0.34em] text-canvas/60">
        ...
        <p className="mt-10 max-w-3xl text-base leading-relaxed text-canvas/68">
          Six flagship case studies lead into the complete ledger: active systems, research, contributions, prototypes, and archived source projects grouped by family, with clear ownership, status, visibility, and scope.
        </p>
      </section>
        ...
        {workProofCards.map((item, index) => {
          ...
          return (
            <article key={item.title} className="group relative grid min-h-[610px] content-between gap-10 overflow-hidden bg-ink p-6 transition-colors duration-300 hover:bg-ink/94 md:p-8">
              <div>
                <div className="flex flex-wrap items-center justify-between gap-4 text-[11px] uppercase tracking-[0.24em] text-canvas/60">
              ...
              <div className="border-t border-canvas/14 pt-6">
                ...
                <div className="mt-6 flex flex-wrap gap-5 text-[11px] uppercase tracking-[0.22em]">
                  <a href={item.href} data-portfolio-cta="work_open_project" target={externalProject ? '_blank' : undefined} rel={externalProject ? 'noreferrer' : undefined} className="inline-flex min-h-11 items-center gap-1.5 border-b border-canvas/28 text-canvas/74 transition-colors hover:border-canvas hover:text-canvas">
                    <span>{item.projectLabel}</span>
                    <span aria-hidden="true">{externalProject ? '↗' : '→'}</span>
                  </a>
                  <a href={item.evidenceHref} data-portfolio-cta="work_open_evidence" target={externalEvidence ? '_blank' : undefined} rel={externalEvidence ? 'noreferrer' : undefined} className="inline-flex min-h-11 items-center gap-1.5 border-b border-accent/40 text-accent transition-colors hover:border-canvas hover:text-canvas">
                    <span>{item.evidenceLabel}</span>
                    <span aria-hidden="true">{externalEvidence ? '↗' : '→'}</span>
                  </a>
                </div>
              </div>
            </article>
                    ...
                    return (
                      <article key={project.id} data-project-family-id={project.id} className="grid gap-6 py-7 md:grid-cols-[4rem_minmax(0,0.62fr)_minmax(260px,0.38fr)] md:px-4">
                        ...
                        <div className="grid content-between gap-5 border-l border-canvas/14 pl-5">
                          <div>
                            <p className="text-[9px] uppercase tracking-[0.2em] text-canvas/48">Scope and limits</p>
      ...
      <section className="relative z-10 mx-auto max-w-[1480px] px-4 py-16 md:px-8 xl:px-10 xl:py-24">
        <div className="mb-10 grid gap-8 lg:grid-cols-[0.38fr_0.62fr]">
          <h2 className="font-serif text-[3rem] italic leading-[0.9] tracking-normal md:text-[4.5rem] xl:text-[6rem]">Methods and working material.</h2>
          <p className="max-w-3xl self-end text-base leading-relaxed text-canvas/68">
            Samples, models, and method notes show how the work was done. They add context to the case studies without pretending to be client outcomes.
          </p>
        </div>
      ...
      >
        <div>
          <p className="text-[11px] uppercase tracking-[0.24em] text-canvas/60">Before we talk</p>
          <h2 id="buyer-decision-title" className="mt-5 max-w-xl font-serif text-[3rem] italic leading-[0.9] tracking-normal md:text-[4.5rem]">
            Three questions sharpen the brief.
          </h2>
          <p className="mt-7 max-w-lg text-sm leading-relaxed text-canvas/64">
            A strong first conversation starts with the decision, the standard for judging the work, and the information already available. The public method and sample let you assess the approach before sharing access.
          </p>
        </div>
        ...
        <ol className="grid gap-px border border-canvas/14 bg-canvas/14">
          {buyerDecisionEvidence.map((item, index) => (
            <li key={item.question} className="group grid gap-6 bg-ink p-5 transition-colors duration-200 hover:bg-canvas/[0.045] motion-reduce:transition-none sm:grid-cols-[3.5rem_minmax(0,1fr)] md:p-7">
              <span aria-hidden="true" className="text-[11px] uppercase tracking-[0.24em] text-accent">
              ...
              <div>
                ...
                <a
                  ...
                  className="mt-5 inline-flex min-h-11 items-center border-b border-canvas/24 text-[11px] uppercase tracking-[0.2em] text-canvas/70 transition-colors hover:border-accent hover:text-accent"
                >
                  {item.action}
                </a>
              </div>
      ...
      <section className="relative z-10 mx-auto max-w-[1480px] border-y border-canvas/14 px-4 py-12 md:px-8 xl:px-10">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(260px,0.34fr)] lg:items-end">
          <div>
            <p className="text-[11px] uppercase tracking-[0.24em] text-canvas/60">Decision ready?</p>
            <h2 className="mt-4 max-w-3xl font-serif text-[2.8rem] italic leading-[0.92] tracking-normal md:text-[4rem]">
              Bring the system, the issue, and the decision.
            </h2>
            <ul className="mt-7 flex flex-wrap gap-x-6 gap-y-3 text-[10px] uppercase tracking-[0.2em] text-canvas/54" aria-label="Useful brief inputs">
              <li>Site or product area</li>
              <li>Observed problem</li>
              <li>Decision and owner</li>
            </ul>
          </div>
          <div>
            <p className="mb-5 max-w-sm text-xs leading-relaxed text-canvas/54">
              Do not send credentials or sensitive client data in the first email. The public method and sample crawl remain open for review.
            </p>
            <a
              href="/contact"
