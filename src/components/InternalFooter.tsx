...
export function InternalFooter({ activePath, tone = 'light' }: InternalFooterProps) {
  ...
  return (
    <footer className={`mx-auto w-full max-w-[1480px] py-4 text-[10px] uppercase ${textMutedClass}`}>
      <div className={`site-footer site-footer-editorial grid gap-4 ${bgClass} px-5 py-5 md:px-7`}>
        <div className="grid gap-5 lg:grid-cols-[minmax(245px,0.58fr)_minmax(0,1.78fr)_minmax(230px,0.56fr)_auto] lg:items-start lg:gap-4">
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <a
                      ...
                      className={`site-footer-link group relative inline-flex min-h-10 items-center justify-center px-3 py-2 leading-none tracking-[0.24em] transition-colors duration-200 ${hoverSurfaceClass} ${active ? textClass : textMutedNavClass}`}
                    >
          ...
          <a
            ...
            className={`site-footer-return group relative inline-flex h-11 w-11 items-center justify-center overflow-hidden border transition-colors duration-200 ${returnClass}`}
          >
            <span aria-hidden="true" className="absolute inset-x-1 bottom-1 h-0 bg-current/10 transition-all duration-300 group-hover:h-[calc(100%-0.5rem)]" />
