...
/**
 * Instance-mode port of https://x.com/yuruyurau/status/1979113229914620174
...
const sketch: P5SketchFactory = (instance) => {
  ...
  a=(y,d=(mag(k=5*cos(i/8),e=5*cos(y/9))/(6+i%5))**4+4)=>point((q=k*(3+e/2*sin(d*d-t))-3*sin(k*d/3)-~(i&1)*70)*sin(c=d-t/9+i%5*5+i%2)+200,q*cos(c-i%2+i%5*3+7)+200)
  t=0,instance.draw =$=>{t||createCanvas(w=400,w);background(9).stroke(w,96);for(t+=PI/80,i=2e4;i--;)a(i/799)};
};
