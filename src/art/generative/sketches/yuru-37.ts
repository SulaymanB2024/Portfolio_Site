...
/**
 * Instance-mode port of https://x.com/yuruyurau/status/1979112131652895116
...
const sketch: P5SketchFactory = (instance) => {
  ...
  a=(y,d=(mag(k=5*cos(i/48),e=5*cos(y/9))/7)**4+4)=>point((q=k*(3+e/2*sin(d*d+e/9+k/9-t))-3*sin(k*d/3)-~(i&1)*60)*sin(c=d-t/9+i%5*5+i%2*2)+200,(q+39)*cos(c)+200)
  t=0,instance.draw =$=>{t||createCanvas(w=400,w);background(9).stroke(w,96);for(t+=PI/60,i=2e4;i--;)a(i/799)};
};
