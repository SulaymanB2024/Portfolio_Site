...
/**
 * Instance-mode port of https://x.com/yuruyurau/status/1977048172347506950
...
const sketch: P5SketchFactory = (instance) => {
  ...
  t=0,d=6e-4,instance.draw =_=>{t++||createCanvas(w=400,w);background(9).stroke(w,66);for(x=y=z=2,i=4e4;i--;point(x*(sin(f=t*PI/40-x*x/99+i%8)+1.5)+99*sin(k=z/69+t*PI/720+i%4*8)+200,200-139*sin(k+1.5)-abs(x*3)*sin(6-f)))[x,y,z]=[x+9*(y-x)*d,y+(x*(28-z)-y)*d,z+(x*y-2*z)*d]};
};
