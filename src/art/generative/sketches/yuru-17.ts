...
/**
 * Instance-mode port of https://x.com/yuruyurau/status/2025114620676186436
...
const sketch: P5SketchFactory = (instance) => {
  ...
  a=(m,d=mag(k=9*cos(i/61),e=i/692-13)**2/99+1)=>point((q=79-e/2*sin(k/d*4)+k/d*(8+5*sin(sin(d*d+e/9-t+m))))*cos(c=d/2+cos(t-d*2+m)/9-t/16+m)+200,(q+40)*sin(c)+200)
  t=0,instance.draw =$=>{t||createCanvas(w=400,w);background(9).stroke(w,96);for(t+=PI/45,i=2e4;i--;)a(i%2*9)};
};
